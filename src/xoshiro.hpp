...
rotl64(const std::uint64_t x, const int k)
{
  return (x << k) | (x >> (64 - k));
}

static inline std::uint32_t
rotl32(const std::uint32_t x, const int k)
{
  return (x << k) | (x >> (32 - k));
}
...
static inline std::uint32_t
extract_32bits_from64_left(const std::uint64_t x)
{
  std::uint32_t out;
  std::memcpy(
    &out, reinterpret_cast<const std::uint32_t*>(&x), sizeof(std::uint32_t));
  return out;
}

static inline std::uint32_t
extract_32bits_from64_right(const std::uint64_t x)
{
  std::uint32_t out;
  std::memcpy(&out,
...
static inline void
assign_32bits_to64_left(std::uint64_t assign_to, const std::uint32_t take_from)
{
  std::memcpy(reinterpret_cast<std::uint32_t*>(&assign_to),
              &take_from,
              sizeof(std::uint32_t));
}

static inline void
assign_32bits_to64_right(std::uint64_t assign_to, const std::uint32_t take_from)
{
  std::memcpy(reinterpret_cast<std::uint32_t*>(&assign_to) + 1,
              &take_from,
              sizeof(std::uint32_t));
}
...
constexpr static const uint32_t JUMP_X128PP[] = { 0x8764000b,
                                                  0xf542d2d3,
                                                  0x6fa035c3,
                                                  0x77f2db5b };

constexpr static const uint32_t LONG_JUMP_X128PP[] = { 0xb523952e,
                                                       0x0b6f099f,
                                                       0xccf5a0ef,
                                                       0x1c580662 };
...
static inline void
jump_state(const int_t jump_table[4], rng_t& rng)
{
  int_t s0 = 0;
  int_t s1 = 0;
  int_t s2 = 0;
...
static inline std::uint64_t
splitmix64(const std::uint64_t seed)
{
...
public:
  ...
  std::uint64_t state[4] = { 0x3d23dce41c588f8c,
                             0x10c770bb8da027b0,
                             0xc7a4c5e87c63ba25,
                             0xa830f83239465a2e };
  ...
  Xoshiro256PP(const std::uint64_t seed) { this->seed(seed); }

  Xoshiro256PP(const std::uint64_t seed[4]) { this->seed(seed); }
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
    ...
    const Xoshiro256PP& e)
  {
    ost.write(reinterpret_cast<const char*>(&e.state[0]),
              sizeof(std::uint64_t));
    ost.put(' ');
    ost.write(reinterpret_cast<const char*>(&e.state[1]),
              sizeof(std::uint64_t));
    ost.put(' ');
    ost.write(reinterpret_cast<const char*>(&e.state[2]),
              sizeof(std::uint64_t));
    ost.put(' ');
    ost.write(reinterpret_cast<const char*>(&e.state[3]),
              sizeof(std::uint64_t));
    return ost;
  }
  template<class CharT, class Traits>
  friend std::basic_istream<CharT, Traits>& operator>>(
    std::basic_istream<CharT, Traits>& ist,
    Xoshiro256PP& e)
  {
...
#pragma GCC diagnostic pop
  ...
  Xoshiro128PP(const std::uint32_t seed) { this->seed(seed); }

  Xoshiro128PP(const std::uint64_t seed) { this->seed(seed); }

  Xoshiro128PP(const std::uint32_t seed[4]) { this->seed(seed); }

  Xoshiro128PP(const std::uint64_t seed[2]) { this->seed(seed); }
